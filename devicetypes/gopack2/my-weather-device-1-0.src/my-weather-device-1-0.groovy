...
*/
metadata {
    definition (name: "My Weather Device 1.0", namespace: "GOPACK2", author: "Gopack2") {
    ...
    tiles {
        ...
        standardTile("alert", "device.alert", inactiveLabel: false, width: 5, height: 2, decoration: "flat", wordWrap: true) {
      		state "[]",icon:"https://github.com/Gopack2/A-Better-Weather/blob/master/Default.png?raw=true", label:'\n${currentValue}'
...
def updated() {
    ...
    def loc1name = location1Name
    log.debug "NAME: " location1Name
	poll()
...
// Since precip_1hr_in is a string, we need to convert it to a decimal in order to compare it as a number.
   ...
   switch (timeFormat) {
        case "12Hour" :
        send(name: "localSunset", value: Date.parse("HH:mm","${astro.sunset.hour}:${astro.sunset.minute}").format( 'h:mm a ' ))
     	send(name: "localSunrise", value: Date.parse("HH:mm","${astro.sunrise.hour}:${astro.sunrise.minute}").format( 'h:mm a' ))
       
        break;
		case "24Hour" :
        send(name: "localSunset", value: Date.parse("HH:mm","${astro.sunset.hour}:${astro.sunset.minute}").format( 'HH:mm ' ))
     	send(name: "localSunrise", value: Date.parse("HH:mm","${astro.sunrise.hour}:${astro.sunrise.minute}").format( 'HH:mm ' ))
        
		break;
        default :
        send(name: "localSunset", value: Date.parse("HH:mm","${astro.sunset.hour}:${astro.sunset.minute}").format( 'h:mm ' ))
     	send(name: "localSunrise", value: Date.parse("HH:mm","${astro.sunrise.hour}:${astro.sunrise.minute}").format( 'h:mm ' ))
            ...
            alerts.each {alert ->
                if (!oldKeys.contains(alert.type + alert.date_epoch)) {
                   ...
                   //send(name: "alert", value: pad(alert.description), descriptionText: msg, isStateChange: true)
                    ...
                    sendEvent(name:"alert", value:"${alertType}")
